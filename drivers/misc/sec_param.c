...
static void param_sec_operation(struct work_struct *work)
{
	...
	if (IS_ERR(filp)) {
		...
		sched_param_data->param_operation_succeed = false;
...
param_sec_debug_out:
	...
	if (ret > 0) 
		sched_param_data->param_operation_succeed = true;
	else
		sched_param_data->param_operation_succeed = false;
	complete(&sched_sec_param_data.work);
	return;
...
bool sec_open_param(void)
{
	...
	if (param_data != NULL) {
		...
		if (sched_sec_param_data.param_operation_succeed == false) {
			ret = false;
			pr_err("%s param file operation fail \n", __func__);
			kfree(param_data);
			param_data = NULL;
		}
	}
...
static int __init sec_param_work_init(void)
{
	...
	sched_sec_param_data.param_operation_succeed=false;
