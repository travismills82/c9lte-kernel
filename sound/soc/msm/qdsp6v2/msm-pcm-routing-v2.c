...
static int msm_sec_cofliter_get(struct snd_kcontrol *kcontrol,
			struct snd_ctl_elem_value *ucontrol)
{
	return 0;
}
...
static int msm_sec_cofliter_put(struct snd_kcontrol *kcontrol,
			struct snd_ctl_elem_value *ucontrol)
{
	int ret = 0;

	mutex_lock(&routing_lock);
	pr_info("%s()  ratation value = %d \n", __func__, (int)ucontrol->value.integer.value[0]);

	ret = adm_set_cofilter_rotation(AFE_PORT_ID_QUATERNARY_MI2S_RX,
...
static const struct snd_kcontrol_new ss_solution_mixer_controls[] = {
	...
	SOC_SINGLE_EXT("Cofilter rotation", SND_SOC_NOPM, 0, 65535, 0,
				msm_sec_cofliter_get, msm_sec_cofliter_put),
};
#endif /* CONFIG_SEC_SND_SOLUTION */
